...
import { SwitchWordRule } from "./SwitchWordRule";
...
describe('Text manipulator', () => {
    ...
    it('should manipulate new text', () => {
        ...
        expect(manipulator.manipulate(getNewScrambledText())).toBe(getNewOriginalText())
    })
    ...
    function getNewScrambledText() {
        return getScrambledText() + ' ' + 'Då remmok de bli regel rätta kod ninjor av rang. King börjar inte ens beskriva hur grymma de blir.'
    }

    function getNewOriginalText() {
        return 'Forefront är här för att stanna. Med stöd från alla Frontiers kommer traineerna mosa allt motstånd. Om två år kommer Forefront förgyllas när traineerna gjort klart utbildningen Code is Queen!' + 

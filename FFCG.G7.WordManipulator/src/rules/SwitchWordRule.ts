...
export class SwitchWordRule extends ManipulateWordBaseRule {
    ...
    manipulate(word: string, index?: number, array?: Array<any>) {
        const config: ISwitchWordConfig = (this.configs as ISwitchWordConfig[]).filter(x => x.old.toLocaleLowerCase() === word.toLowerCase())[0]
